...
import { convertCandidateTextToObject } from '../helper/score.js';
...
export const extractSkills = async (data) => {
...
export async function generateSkillAssessment(candidates) {
    const prompt = `
Input:
...
0-5 (Entry-level): Responsibilities are vague or not clearly articulated. Keywords used seem generic without significant context or results. The candidate's involvement appears minimal or limited to non-technical tasks.

6-15 (Intermediate): The candidate shows more detail in their experience with specific tools or technologies. They are able to articulate challenges and outcomes, but their role may still be more focused on execution rather than leadership or innovation.

16-25 (Advanced): The candidate demonstrates deep expertise, takes initiative, and has measurable impact. They work independently on complex tasks, contribute to strategic decisions, and have leadership or mentoring responsibilities.
...
export async function generatePotentialAssessment(jobDescription, candidates) {
    const prompt = `
Input:
...
6-15 (Intermediate): The candidate provides more specific details about their experience with tools or technologies. They describe challenges and outcomes but are still more focused on execution rather than leadership or innovation.

16-25 (Advanced): The candidate shows deep expertise, initiative, and measurable impact. They work independently on complex tasks, contribute to strategic decisions, and hold leadership or mentoring roles.
...
export async function generateWhyHeIsBestCandidate(experienceScoredCandidates, potentialFitScoredCandidates, candidatesWithTotalScore, bestCandidate, role) {
