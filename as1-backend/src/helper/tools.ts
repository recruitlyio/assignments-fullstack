...
export const createNode = tool(
  async (input: { label: string; properties: Record<string, any> }): Promise<string> => {
    ...
    try {
      const result = await session.run(
        `MATCH (a),(b)
         ...
         RETURN id(r) as id`,
        { start: startId, end: endId, props: properties }
      )
      const record = result.records[0]
      return record ? record.get('id').toString() : ''
    } finally {
...
export const getNode = tool(
  async (input: { nodeId: number }): Promise<{ labels: string[]; properties: Record<string, any> } | null> => {
    const { nodeId } = input
    const session = neo4jDriver.session()
    try {
      const result = await session.run(
...
export const getRelations = tool(
  async (input: { nodeId: number; relType?: string }): Promise<Array<{ targetId: number; type: string; properties: Record<string, any> }>> => {
