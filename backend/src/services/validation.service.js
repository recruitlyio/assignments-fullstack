...
exports.validateAndStandardizeResume = void 0;
...
const canonicalSkillEmbeddings = new Map();
const canonicalDegreeEmbeddings = new Map();
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (CANONICAL_SKILLS.length > 0) {
                ...
                const skillResult = yield model.batchEmbedContents({ requests: skillRequests });
            ...
            if (CANONICAL_DEGREES.length > 0) {
                ...
                const degreeResult = yield model.batchEmbedContents({ requests: degreeRequests });
 ...
 */
function findBestMatch(inputText_1, canonicalEmbeddings_1) {
    return __awaiter(this, arguments, void 0, function* (inputText, canonicalEmbeddings, similarityThreshold = DEFAULT_SIMILARITY_THRESHOLD) {
        var _a;
        // Basic input check
        ...
        try {
            ...
            const apiKey = process.env.GEMINI_API_KEY;
            if (!apiKey)
                throw new Error('GEMINI_API_KEY not set'); // Should be caught earlier, but good practice
            const genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
            const model = genAI.getGenerativeModel({ model: EMBEDDING_MODEL_NAME });
            const result = yield model.embedContent(trimmedInput);
            const inputEmbedding = (_a = result.embedding) === null || _a === void 0 ? void 0 : _a.values;
            if (!inputEmbedding) {
 ...
 */
const validateAndStandardizeResume = (rawData) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (rawData.skills && Array.isArray(rawData.skills)) {
        ...
        for (const skill of rawData.skills) {
            if (skill.name) {
                const standardizedName = yield findBestMatch(skill.name, canonicalSkillEmbeddings, DEFAULT_SIMILARITY_THRESHOLD);
                const finalName = standardizedName || skill.name; // Use original if no match
                standardizedSkills.push(Object.assign(Object.assign({}, skill), { name: finalName }));
    ...
    if (rawData.education && Array.isArray(rawData.education)) {
        const standardizedEducation = [];
        for (const edu of rawData.education) {
            let standardizedDegree = edu.degree; // Default to original
            let noteAdded = false;
            if (edu.degree) {
                const matchedDegree = yield findBestMatch(edu.degree, canonicalDegreeEmbeddings, DEFAULT_SIMILARITY_THRESHOLD);
                if (matchedDegree) {
            ...
            standardizedEducation.push(Object.assign(Object.assign({}, edu), { degree: standardizedDegree }));
        }
    ...
    if (rawData.workExperience && Array.isArray(rawData.workExperience)) {
        validatedData.workExperience = rawData.workExperience.map((exp, index) => {
            let validatedExp = Object.assign({}, exp); // Copy
...
exports.validateAndStandardizeResume = validateAndStandardizeResume;
